...
<div class="pet-frame"
     ...
     data-owner="{{ $pet->owner->first_name ?? 'N/A' }} {{ $pet->owner->last_name ?? 'N/A' }}"
    ...
    @if($isAdmin)
        ...
        <div class="pet-mini-info" style="display:flex; flex-direction:row; justify-content:center; gap:10px; padding:30px 10px 15px 10px;">
            <button type="button"
                    ...
                    onclick="event.stopPropagation(); showAdminPetInfo(this.closest('.pet-frame'))"
    ...
    @else
        ...
        <div class="pet-mini-info">
            <p>{{ $pet->age }}</p>
            <p>{{ $pet->gender }}</p>
            <p hidden>{{ $pet->owner->first_name ?? 'N/A' }} {{ $pet->owner->last_name ?? 'N/A' }}</p>
        </div>
    @endif
